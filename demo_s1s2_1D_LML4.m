...
%% Control Variables
experiment=4; % Set manually the number of the experiment, you can check
...
while(flag_modelup)
    ...
    % Sorting lml
    min_point=min(lml_test);
    [value,idx]=min(lml_test);
    ...
    if(LMLTEST(GP_opt,GP_opt_plus(idx))>0)
        ...
        X_consistent=union(X_consistent,X_test(idx),'legacy');
        y_consistent= union(y_consistent,y_test(idx),'legacy');
        tc=tc+1;
        tested_index(tc)=p;
        flag_modelup=1;
    else
